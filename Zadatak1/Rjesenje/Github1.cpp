...
ostream& operator << (ostream& o, eRazred razred) {
	if (razred == PRVI) o << "PRVI";
	else if (razred == DRUGI) o << "DRUGI";
	else if (razred == TRECI) o << "TRECI";
	else if (razred == CETVRTI) o << "CETVRTI";
	return o;
}
const float minimalanProsjek = 4.5;
...
public:
	...
	Kolekcija(const Kolekcija& kolekcija) {
		...
		_elementi1 = new T1[*kolekcija._trenutno];
		_elementi2 = new T2[*kolekcija._trenutno];
...
public:
	...
	DatumVrijeme(const DatumVrijeme& datum) :DatumVrijeme(*datum._dan, *datum._mjesec, *datum._godina, *datum._sati, *datum._minuti) {}
	...
	bool operator==(const DatumVrijeme& datum) {
		return
			*_dan == *datum._dan &&
			*_mjesec == *datum._mjesec &&
			*_godina == *datum._godina &&
...
public:
	...
	Predmet(const Predmet& predmet) :Predmet(predmet._naziv, predmet._ocjena, predmet._napomena) {}
	~Predmet() {
	...
	bool operator == (const Predmet& predmet) { return strcmp(_naziv, predmet._naziv) == 0; }
};
class Uspjeh {
	eRazred _razred;
...
public:
	...
	Uspjeh(Uspjeh& uspjeh) :Uspjeh(uspjeh._razred) {
		_predmeti = new Kolekcija<Predmet*, DatumVrijeme>;
		if (uspjeh.GetPredmeti() != nullptr) {
			for (size_t i = 0; i < uspjeh.GetPredmeti()->getTrenutno(); i++)
			{
				_predmeti->AddElement(new Predmet(*uspjeh.GetPredmeti()->getElement1(i)), uspjeh.GetPredmeti()->getElement2(i));
			}
		}
	}
	...
	bool AddPredmet(eRazred razred, Predmet predmet, DatumVrijeme datum) {
		for (vector<Uspjeh*>::iterator i = _uspjeh.begin(); i != _uspjeh.end(); i++)
		{
			...
			if (u->GetRazred() == razred) {
				...
				{
					if (*u->GetPredmeti()->getElement1(j) == predmet || u->GetPredmeti()->getElement2(j) == datum) return false;
				}
...
#pragma region TestiranjeKandidata
	...
	*/
	...
	sa porukom: "evidentirali ste uspjeh za X razred". ukoliko je prosjek na nivou tog razreda veci od minimalanProsjek
